  ...
  const resp = await withErrorReporting(
    withAuthHandler(
      fetch("https://api.hackercamp.cz/v1/admin/registrations", {
        ...
        body: JSON.stringify({
          command: "invoiced",
          params: { registrations: emails.map(email => ({ year, email })), invoiceId: data.id }
        }),
        credentials: "include",
        mode: "cors"
      }),
  ...
  if (subsById.size > 0) {
    for (const [id, subject] of subsById) {
      ...
      const identity = `${subject.name} ${
        subject.registration_no
          ? `(IČO: ${subject.registration_no}${subject.vat_no ? `; DIČ: ${subject.vat_no}` : ""})`
          : ""
      } `;
