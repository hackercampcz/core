...
function getTravel(travel) {
  switch (travel) {
    case "carpool":
      return ["A potřebuje na camp hodit. Máš místo v autě? Domluvíte se v kanále #spolujizda?"];
    case "free-car":
      return ["A navíc má místo v autě a nabízí odvoz! Domluvíte se v kanále #spolujizda?"];
    default:
      return [];
  }
}
...
export function attendeeAnnouncement({ image, name, slackID, travel }) {
  return [{
    ...
    text: {
      type: "mrkdwn",
      text: [`Hey! <@${slackID}> s námi letos jede na camp.`].concat(getTravel(travel)).concat(getActions()).join("\n")
    },
    accessory: { type: "image", image_url: image, alt_text: name }
  }];
}
...
export async function postChatMessage(channel, message) {
  ...
  if (channel.startsWith("hc-")) {
    console.log({ msg: "synthetic users can't receive slack messages", channel });
    return null;
  }
