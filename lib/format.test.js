...
describe("formatShortDate", () => {
  ...
  it("returns a non-empty string for a valid date", () =>
    expect(formatShortDate(REF_DATE).length).toBeGreaterThan(0));
});

describe("formatTime", () => {
  it("returns null for null", () => expect(formatTime(null)).toBeNull());
  it("contains hours and minutes separated by :", () =>
    expect(formatTime(REF_DATE)).toMatch(/\d{1,2}:\d{2}/));
});
...
describe("formatDateTime", () => {
  ...
  it("returns a non-empty string for a valid date", () =>
    expect(formatDateTime(REF_DATE).length).toBeGreaterThan(0));
});
