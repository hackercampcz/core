...
export function getRegistrationProjection() {
  return ({ firstName, lastName, timestamp, invoiced, paid, firstTime, referral, ticketType, approved, ...rest }) => {
    ...
    return Object.assign(rest, {
      ...
      invoicedAt: invoiced ? new Date(invoiced).getTime() : null,
      paidAt: paid ? new Date(paid).getTime() : null,
      _tags: getTags(rest.year.toString(), paid, invoiced, isConfirmed, isVolunteer, isStaff)
    });
  };
}
...
export function getAttendeesProjection() {
  return ({ paid, housing, travel, ticketType, nfcTronData, ...rest }) =>
    Object.assign(rest, {
      nfcSN: nfcTronData.map(x => x.sn),
      objectID: `${rest.year}-${rest.slackID}`,
      createdAt: new Date(paid).getTime(),
      _tags: [rest.year.toString(), travel, housing, ...getAttendeeTicketTags(ticketType)].filter(Boolean)
    });
}
